...
export default class HomeScreen extends Component {
  constructor(props) {
    ...
    this.state={
      movieDetails: {},
      ngrok_url:"https://912b-122-161-241-197.ngrok-free.app/movies"
    }
  }

  /*define getmovie(), likedMovie(), dislikedMovie() ,notWatched() functions here*/
  componentDidMount(){
    //ComponentDidMount is used to initialize the app and the code (When the app boots)
    /// For expample, the given function getMovie() runs automatically. Any function specified will run automatically
    this.getMovie()
  }
  getMovie = () => {
    const MoviesUrl = this.state.ngrok_url+"/movies"
  ...
  likedMovie = () => {
    const MoviesUrl = this.state.ngrok_url+"/like"
    axios.get(MoviesUrl).then(
      (response) => {
        this.getMovie()
      }
  ...
  dislikedMovie = () => {
    const MoviesUrl = this.state.ngrok_url+"/dislike"
    axios.get(MoviesUrl).then(
      (response) => {
        this.getMovie()
      }
  ...
  notWatched = () => {
    const MoviesUrl = this.state.ngrok_url+"/notWatch"
    axios.get(MoviesUrl).then(
      (response) => {
        this.getMovie()
      }
            ...
            <View style={styles.subContainer}>
              ...
              <View style={styles.ratingContainer}>
                {/*Add the components to show rating of the movie below*/}
                  <Star score={rating} style={styles.starStyle}></Star>
              </View>
              <View style={styles.iconButtonContainer}>
                ...
                <TouchableOpacity onPress={this.notWatched}>
                  <Image source={require("../assets/didNotWatch.png")} />
                </TouchableOpacity>
              </View>
            </View>
          </ImageBackground>
